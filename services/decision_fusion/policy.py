...
from services.common.otel import current_correlation_id, start_span
...
def _directional_price_gate(item: DecisionInput) -> tuple[bool, str]:
    if item.direction.upper() == "LONG":
        valid = item.stop < item.entry and all(
            target > item.entry for target in item.targets
        )
...
def _ml_gate(item: DecisionInput, policy: FusionPolicy) -> tuple[bool, str, bool]:
    ...
    if not calibrated_real:
        return (
            True,
            "ML output is score-only/non-real-market calibrated and is non-decisive",
            False,
        )
...
def _evidence_score(item: DecisionInput, ml_used: bool) -> float:
    ...
    expectancy_quality = max(
        0.0, min((item.historical_expectancy_r + 0.2) / 0.7, 1.0)
    )
    weighted = [
...
def fuse_decision(
    ...
    *,
    ...
    correlation_id = correlation_id or current_correlation_id() or str(uuid.uuid4())
    with start_span(
        "decision.fusion",
        attributes={
            "tradeops.execution_mode": execution_mode.value,
            "tradeops.regime": item.regime,
            "tradeops.risk_status": item.risk_status,
            "tradeops.policy_version": policy.version,
        },
    ) as span:
        ...
        gates = (
            (
                "assessment",
                item.assessment_status.upper() == "SUPPORTED",
                "I6 assessment must be SUPPORTED",
            ),
            (
                "risk",
                item.risk_status.upper() == "ACCEPT",
                "I3 deterministic risk gate must be ACCEPT",
            ),
            (
                "freshness",
                item.event_age_ms <= freshness_limit,
                f"event age must be <= {freshness_limit:.0f}ms",
            ),
            (
                "evidence_quality",
                item.evidence_quality >= policy.min_evidence_quality,
                f"evidence quality must be >= {policy.min_evidence_quality:.2f}",
            ),
            (
                "regime",
                item.regime.upper() in policy.allowed_regimes,
                "regime must be explicitly allowed by fusion policy",
            ),
            ("rr", item.rr >= policy.min_rr, f"R/R must be >= {policy.min_rr:.2f}"),
            (
                "historical_sample",
                item.historical_trade_count >= policy.min_historical_trades,
                f"historical trade count must be >= {policy.min_historical_trades}",
            ),
            (
                "historical_expectancy",
                item.historical_expectancy_r >= policy.min_historical_expectancy_r,
                (
                    "historical expectancy R must be >= "
                    f"{policy.min_historical_expectancy_r:.3f}"
                ),
            ),
        ...
        failed = [
            f"{name}: {reason}" for name, passed, reason in gates if not passed
        ]
        for name, passed, _ in gates:
            if not passed:
                DECISION_GATE_FAILURES.labels(gate=name).inc()
        ...
        DECISION_PROPOSALS.labels(
            decision=decision.value, execution_mode=execution_mode.value
        ).inc()
        span.set_attribute("tradeops.decision", decision.value)
        span.set_attribute("tradeops.failed_gate_count", len(failed))
        span.set_attribute("tradeops.evidence_score", evidence_score)
        span.set_attribute("tradeops.ml_contribution_used", ml_used)
        rationale = (
            tuple(failed)
            if failed
            else (
