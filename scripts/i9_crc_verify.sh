...
for name in market-data workflow-api genai-api rag-api agent-controller mcp-server risk-engine paper-oms notifier otel-collector prometheus grafana; do
...
host="$(oc -n "$namespace" get route agent-controller -o jsonpath='{.spec.host}')"
curl -fsSk "https://${host}/health" >/dev/null
